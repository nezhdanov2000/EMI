...
#### 9.2.2. Визуальное кодирование массы центра (Mass Encoding via Marker Radius)

Каждый дискретный центр агрегирует $N_{cell} \ge 1$ объектов исходного датасета. Для передачи статистической значимости (массы) центра используется суб-линейное масштабирование радиуса маркера:

$$\text{Radius}(\mathbf{c}) \propto \sqrt{\log(1 + N_{cell}(\mathbf{c}))}$$

Суб-линейность гарантирует, что:
* Редкие центры ($N_{cell} = 1\text{–}5$) остаются визуально различимыми и не исчезают;
* Массивные центры ($N_{cell} = 1000+$) выделяются размером, но не поглощают всё визуальное пространство;
* Аналитик мгновенно отличает глобальные закономерности (крупные сферы) от статистических аномалий (мелкие сферы).
