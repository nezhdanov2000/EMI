...
### 4.5. Универсальный скрининг зависимостей и пропозиционализация (Universal Propositional Screening & Auto-Discovery)

Для автономного выявления всех фундаментальных закономерностей датасета без необходимости ручного выбора целевой переменной VSF расширяет алгоритм AVR до сквозного исследовательского движка:

1. **Генерация атомарных предикатов (Propositionalization / One-vs-Rest):**
   Для каждой характеристики датасета $X_k$ ($k=1..M$):
   - Если $X_k$ бинарен, он напрямую выступает в роли целевого концепта $Z = X_k$;
   - Если $X_k$ многоклассовый (например, *«цвет шляпки»* с градациями: *коричневый, желтый, белый и др.*), система формирует семейство бинарных предикатов:
     $$Z_{k, c} = \mathbb{I}[X_k = c], \quad \forall c \in \text{Categories}(X_k)$$
     (например, гипотеза *«коричневая шляпка» vs «все остальные»*).

2. **Двухступенчатый конвейер отбора (Two-Stage Screening Pipeline):**
   - **Ступень 1 (Fast Vectorized Filter):** Векторизованный предварительный расчет одномерной Взаимной Информации $I(Z_{k,c}; X_j)$ для всех сформированных пар за $O(M \cdot K_{total})$. Отсечение предикатов со слабой зависимостью ($I < \epsilon$).
   - **Ступень 2 (Full AVR Execution):** Для перспективных предикатов запускается полный цикл алгоритма AVR (Раздел 4.4) с пермутационным тестированием ($p < 0.01$) и поиском многомерных синергетических подмножеств осей $S^* \subseteq \mathcal{V}$.

3. **Формирование каталога фундаментальных инсайтов (Automated Insight Catalog):**
   Найденные зависимости ранжируются по силе связи ($VIR$ и $NMI$), формируя упорядоченный реестр законов датасета:
   - *Инсайт 1:* «Запах (odor) на 90.7% детерминирует съедобность (class) при $d^* = 1$».
   - *Инсайт 2:* «Предикат [споровый порошок = белый] на 96.4% детерминируется 3D-пространством [ножка + кольцо]».
   - Пользователь получает возможность кликнуть на любой инсайт и мгновенно перестроить интерактивную 7D-проекцию под выбранный закон.
