...
* **Post-selection check:** the per-schema certificate drawn by the interface holds for a schema fixed in advance, not for the one the search reports — on pure noise at least one reported branch carries a "certified" centre in 68 % of runs. `vsf.certify_discovery` gives certificates that hold for the reported schemas (Bonferroni over every cell of every scored schema, or a search half and an evaluation half), and `vsf.nested_crossvalidation` gives the out-of-sample coverage of the whole search with the stability of the chosen schema. In the app: Branches → *Check this result* (`/api/validate`, background job); the lattice itself still shows the per-schema certificate, and the static export does not carry the check. Section 4.14.
