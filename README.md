...
* **Rules tab:** every cell of the schemas on the τ-curves (the best of its dimensionality at some purity floor; the four winners among them) read as a rule — `passenger_class = 2nd_Class ∧ sex = male ∧ age_group = child`: 9 rows, 100 % of them survived, certified at the current boundary — with, for each condition, what the rule says without it (`sex = male` alone: 15.7 % of 108 rows, so the age added +84 pp). Refinements whose share is *lower* than their parent's are kept on purpose: learning more changed the probability, and that is the finding. Tick which dimensionalities to list, set a minimum cell size, and narrow the list by the conditions a rule contains — characteristic, then value, then the next characteristic; *Show* lists every rule containing the fixed conditions, bare or refined, *Show all* resets. The bars count the listed rules by share of the value in ten-percent intervals, each bar split by dimensionality; click an interval's label to see its cards. Erasing: in the right panel the τ-curves replace the certificate controls — pick a branch, click a curve at a floor, and the schema there is erased (its points become gaps, nothing replaces them; its rules leave the list); a coloured bar segment erases its schema the same way; chips, Undo, Reset; nothing statistical changes (`/api/rules`; `vsf.rules.enumerate_rules`). Section 4.15.
