"""
False-certificate rate of the REPORTED winner under a global null.
...
def one_run(cfg: NullConfig, seed: int) -> Dict[int, bool]:
    """Return, per dimensionality, whether the winner certifies >= 1 centre."""
...
def main() -> None:
    ...
    hits: Dict[int, int] = {d: 0 for d in range(1, cfg.max_d + 1)}
    t0 = time.perf_counter()
    for seed in range(args.runs):
        for d, hit in one_run(cfg, seed).items():
            hits[d] += int(hit)
    ...
    for d, k in hits.items():
        ...
        rows.append({"d": d, "runs": args.runs, "false_certificates": k,
                     "rate": k / args.runs, "ci95_low": lo, "ci95_high": hi,
                     "nominal": cfg.alpha, **cfg.__dict__})
        print(f"d={d}: {k}/{args.runs} = {k/args.runs:.2f}  "
