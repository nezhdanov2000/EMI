...
let allColumns = [];
let defaultTargetId = null;  // from /api/columns' default_target — the dataset's actual configured target, not a hardcoded "class"
let activeInputs = {};
let graphData = null;
let currentPhase = 0;
let phaseTimer = null;
...
async function fetchColumns() {
    try {
        ...
        // The server tells us which column is the dataset's configured
        // default target (vsf.serve's `default_target` — "class" for the
        // mushroom demo, but arbitrary for any other DataFrame). Using this
        // instead of a hardcoded 'class' is what lets this same bundled
        // frontend serve any dataset via vsf.serve(df).
        defaultTargetId = data.default_target;
        ...
        allColumns.forEach(col => {
            ...
            if (col.id === defaultTargetId) optT.selected = true;
...
function renderFinalSummary() {
    ...
    // Verdict banner: this used to color green/"edible" specifically when
    // `target_criterion === 'e'` — a UCI-Mushroom-only assumption (there is
    // no dataset-agnostic notion of which class value is the "good"
    // outcome). Styling is neutral now, identical for every target/dataset.
...
async function mineGlobalLinks() {
    ...
    resultsContainer.innerHTML = '<div class="kb-empty-state">Поиск статистически значимых цепочек рассуждений через медиаторов (пермутационный тест + FDR-коррекция)... ⏳</div>';
...
function renderKBResults(data) {
    ...
    if (!data.top_chains || data.top_chains.length === 0) {
        container.innerHTML = '<div class="kb-empty-state">Статистически значимые цепочки не найдены для выбранных условий (все звенья должны пройти пермутационный тест с FDR-коррекцией). Попробуйте снизить порог NMI.</div>';
        return;
    }
    ...
    countBanner.innerHTML = `Найдено <b>${data.total_found}</b> статистически значимых цепочек (мин. NMI звена ≥ порога, FDR q ≤ ${(data.fdr_q ?? 0.05)}) для цели <b>${data.target_label}</b>:`;
    ...
    data.top_chains.forEach((chain, idx) => {
        ...
        let gainHtml = '';
        if (chain.gain > 0.001) {
            gainHtml = `<span style="color: #10b981; font-weight: bold; font-size: 0.75rem;">+${gainPct} к прямому измерению${ratioText}</span>`;
        } else {
            gainHtml = `<span style="color: #a5b4fc; font-size: 0.75rem;">Сопоставимо с прямым путём</span>`;
        }
